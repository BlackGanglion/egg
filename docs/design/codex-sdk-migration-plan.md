...
- 对外只暴露主 agent：Linear 触发和直接聊天都进入主 agent，再由主 agent 分发给子 agent。
...
- 保留分诊业务能力：`linear` 子 agent 收集 Linear issue 上下文、读取自己工作区内的 `prompts/triage.md`、必要时读取 Langfuse trace，并由同一个 `linear` 子 agent 写回 Linear。
- 支持多个 Linear issue / AgentSession / 直接聊天 session 同时触发：不同 session 可以并发运行，同一 session 内必须串行化或显式取消旧 run。
...
| Linear coupling | `MainAgent` / `IssueTriage` 直接依赖 `LinearApiClient` | 仅 `linear` 子 agent 依赖 `LinearApiClient`；adapter/bridge/main agent 不读写 Linear API |
| Public entry | Webhook 可直接触发子 agent | 所有外部入口统一调用 `MainAgent.dispatch()` |
| Session state | Linear session 历史由 `MainAgent` 临时拉取 | 统一维护 `ExternalSessionRef -> agentSessionId -> codexThreadId` |
| Prompt ownership | 全局 `prompts/triage.md` | 移到 `linear` 子 agent 工作区，作为子 agent 可迭代资产 |
...
- `src/agent/sub/<agent-name>/workspace/`：子 agent 自有工作区，保存 prompt、评估样例、迭代记录和只属于该子 agent 的上下文资产。
...
每个子 agent 都可以有自己的 workspace。workspace 是该子 agent 的长期资产目录，Codex run 可以在受控权限下读取或修改其中的 prompt、评估样例和迭代记录。

建议结构：

```text
src/agent/sub/linear/
  index.ts
  triage.ts
  client.ts
  workspace/
...
`linear` 子 agent 的分诊 prompt 从全局 `prompts/triage.md` 迁移到 `src/agent/sub/linear/workspace/prompts/triage.md`。主 agent 分发 `linear.issue.triage` task 时，只告诉 `linear` 子 agent 执行任务；`linear` 自己通过 `LinearApiClient` 读取 issue/team/member/label/state/session 上下文，自己从 workspace 加载 prompt，并把该 workspace 作为 Codex thread 的受限工作区。

自迭代规则：

- `linear` 可以在自己的 workspace 中维护和修改 `prompts/triage.md`。
- prompt 迭代必须配套更新 `workspace/evals/` 中的样例或评估说明，避免只凭单次反馈改 prompt。
- 运行分诊时默认只读 prompt；只有明确的“优化/迭代 linear triage prompt”任务才允许 Codex 写 workspace。
- 子 agent 不能修改其他子 agent 的 workspace，也不能把 Linear API token、OAuth token 等外部凭证写入 workspace。
...
- 同一 `ExternalSessionRef` 继续使用同一个 `codexThreadId`，从而保留聊天记录和上下文。
...
5. 为子 agent 创建 `workspace/`，把该子 agent 自有 prompt、eval cases、迭代记录放进去。
6. 如果子 agent 需要外部系统数据，只通过 task context 或专门的 owning sub-agent 传入，不直接 import 外部 SDK；例外是平台 owning sub-agent，例如 `linear` 子 agent 可以使用 `LinearApiClient`。
...
- 通过 `LinearApiClient` 读取 Linear issue、team、member、label、workflow state、comments、AgentSession activities。
...
- 维护 Linear 专属工具，例如 `fetch_linear_issue_context`、`submit_triage_result`、`send_linear_activity`。
- 仅 `linear` 子 agent 可以 import `@linear/sdk` / `LinearApiClient`；主 agent、bridge、adapter、其他子 agent 都不能直接读写 Linear。
...
| 外部对象 | Agent 对象 | 用途 |
| --- | --- | --- |
| `linear.issueId` | `agentSessionId` / `agentRunId` / `codexThreadId` | 自动分诊去重、结果回写到正确 issue |
| `linear.agentSessionId` | `agentSessionId` / `codexThreadId` | Linear 多轮对话上下文恢复、同 session 串行化 |
| `linear.agentActivityId` | `agentTurnId` | Linear prompted 事件去重、避免重复回复 |
| `directChat.conversationId` | `agentSessionId` / `codexThreadId` | 直接聊天上下文恢复、同 conversation 串行化 |
...
| --- | --- | --- | --- |
| Issue 自动分诊 | `triage:${issueId}` | 每个 issue 新建独立 thread | 不同 issue 可并发；同一 issue 去重 |
| Webhook gap 补漏 | `triage:${issueId}` | 与普通分诊一致 | 进入同一队列，避免补漏瞬间打满 |
| Linear AgentSession | `session:${agentSessionId}` | 每个 Linear session 绑定一个可恢复 thread | 不同 session 可并发；同一 session 串行处理 prompted |
| 直接聊天 | `chat:${channel}:${conversationId}` | 每个聊天 conversation 绑定一个可恢复 thread | 不同 conversation 可并发；同一 conversation 串行处理 message |

### RunCoordinator 职责

- 维护 `activeRuns: Map<string, ActiveRun>`，记录 `runKey`、`agentRunId`、`codexThreadId`、`AbortController`、开始时间、来源和状态。
...
active run 可以只保存在内存中；外部 session 到 Codex thread 的映射应持久化到 `.data/agent-session-store.json`。这样服务重启后仍能把同一个 Linear AgentSession 或直接聊天 conversation resume 到原 Codex thread。
...
8. 明确 `SubAgentWorkspace` 合同，至少包含 `workspacePath`、`promptPath`、`evalsPath` 和读写权限模式。
...
3. 将 Linear 读写从 bridge/adapter 移入 `linear` 子 agent：bridge 只生成 `linear.*` task，`linear` 子 agent 负责读 issue/session 并写回 Linear。
4. 新增 `AgentSessionStore`，维护 `issueId` / `agentSessionId` / `agentActivityId` / `conversationId` / `messageId` 与 `agentRunId` / `codexThreadId` 的映射。
5. 此阶段完成后，除 `src/agent/sub/linear/**` 外，`src/agent/**` 不应 import `@linear/sdk` 或 `LinearApiClient`。
...
2. 将当前全局 `prompts/triage.md` 迁移到 `src/agent/sub/linear/workspace/prompts/triage.md`。
3. `LinearTriageSubAgent.invoke(task)` 从自己的 workspace 加载 `prompts/triage.md`，只负责构造 prompt、图片/文件输入和工具列表，不再直接创建第三方 Agent。
4. `CodexRunner` 执行 `linear` 子 agent 时将子 agent workspace 作为受限工作区；普通分诊 run 使用只读模式，prompt 自迭代 run 才允许写入。
5. `submit_triage_result` 继续作为最终提交工具，由 `linear` 子 agent 内部通过 `LinearApiClient` 写回 Linear，并返回结构化 `TriageResult`。
...
5. 同一个 Linear AgentSession 的 `created` / `prompted` 通过 session store 找到同一个 Codex thread，并使用同一个 session queue，避免用户连续追问时并行 run 互相覆盖最终回复。
...
4. 手动验收：
   ...
   - `linear` 从 `src/agent/sub/linear/workspace/prompts/triage.md` 加载分诊 prompt。
   - 普通分诊 run 不能写 prompt；明确的 prompt 迭代 run 可以在 `linear` workspace 内修改 prompt 和 evals。
   - Linear AgentSession 可收到 thought/response/error。
   - 同时触发多个 issue 时，分诊结果写回各自 issue，不串线。
   ...
   - 除 `src/agent/sub/linear/**` 外，`src/agent/**` 不依赖 Linear SDK；bridge/adapter 不调用 `LinearApiClient`。
...
- `linear` 的 triage prompt 是该子 agent workspace 中的文件资产，可在受控任务中自迭代。
- 新子 agent 可以通过统一 `SubAgent` 合同和 capability 注册扩展，不反向依赖 Linear。
- 多个 Linear issue / AgentSession / 直接聊天 session 同时触发时，Codex run 彼此隔离，受全局并发上限控制。
